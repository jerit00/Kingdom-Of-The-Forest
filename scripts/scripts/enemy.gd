...
@export var attack_range: float = 50.0  # Rango de ataque ajustado para detenerse antes
...
func _ready():
	...
	$Hitbox_Enemy.body_entered.connect(_on_collision_with_player)
...
func _on_collision_with_player(body: Node):
	if body == player:
		# Añadir un pequeño empuje para separarse
		var separation_vector = (position - player.position).normalized() * 100
